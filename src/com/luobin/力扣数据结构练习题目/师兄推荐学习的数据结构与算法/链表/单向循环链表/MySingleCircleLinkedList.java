package com.luobin.力扣数据结构练习题目.师兄推荐学习的数据结构与算法.链表.单向循环链表;

import com.luobin.力扣数据结构练习题目.接口的定义.AbstractList;

public class MySingleCircleLinkedList<E> extends AbstractList<E> {
    // 链表的大小
    private int size;
    ...
    public void add(int index, E element) { // O(n)
        ...
        if (index == 0) {
            ...
            // 单向循环链表，将最后面的元素指向链表的最前面，在 0 出进行做文章，所以在这里处理
            // 拿到最后一个节点
            // 当在链表的最前面添加数据，当元素个数为 0 的时候，last = first ,否则是 node(size - 1)
